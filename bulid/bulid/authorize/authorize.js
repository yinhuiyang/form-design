...
(function () {
  var authorize = {
    ...
    loadbind: function loadbind(id) {
      var _this2 = this;

      var $html = $(this.formBoxHTml);
      var iddate = new Date();
      ...
      form.panels.forEach(function (element) {
        if (element.name == '') {
          eldata = {};
        ...
        var $formHtml = _this2.uphtml[element.type](element, eldata, elattribute);
        if (element.type !== 'table') {
          $formHtml = _this2.addChild($formHtml, element, eldata, elattribute);
        }
        // $formHtml.find('fieldset').html(childHtml)
        _this2.addHTml($html.find('.form').attr('id'), $formHtml);
        _this2.regFn('#' + $formHtml.find('form').attr('id'));
      });
    },
    loadAction: function loadAction(idClass) {
      var _this3 = this;

      var actions = this.formData.actions;
      var id = this.$formBoxHTml.find('.form_btn').attr('id');
      var btn = '<button type="button" class="am-btn am-btn-default am-radius" style="margin-right: 5px;" id=""></button>';
      actions.forEach(function (v) {
        ...
        _this3.actionClick($btn, v, idClass);
        $('#' + id).append($btn);
      });
    },
    actionClick: function actionClick($btn, action, idClass) {
      var _this = this;
      if (idClass) {
    ...
    ifFile: function ifFile() {
      var errHtml = '<div class="am-alert am-alert-danger" style="display: block;">\u8BF7\u9009\u62E9\u6587\u4EF6</div>';
      var err = false;
    ...
    addHTml: function addHTml(id, html) {
      $('#' + id).append(html);
    },
    addChild: function addChild($formHtml, elem, eldata, elattribute) {
      var _this4 = this;

      var grid = 0;
      var gridHtml = '<div class="am-g"></div>';
      var $gridHtml = $(gridHtml);
      elem.content.forEach(function (value, i) {
        if (!value.grid) {
          value.grid = 12;
        }
        ...
        var $child = _this4.uphtml[value.type](value, eldata, elattribute);
    ...
    uphtml: {},
    uphtmlFn: function uphtmlFn(type, fn) {
    ...
    getdataFn: function getdataFn(id) {
      ...
      $('#' + id).children('.group').each(function (i, elem) {
        if ($(elem).attr('data-xhtml') == 'table') {
          ...
          !$(elem).attr('name') ? Object.assign(data, _this.getdatachild(elem)) : data[$(elem).attr('name')] = _this.getdatachild(elem);
        }
      });
      // console.log(data)
      return data;
    },
    getdataTable: function getdataTable(elem) {
      var _this = this;
      var data = {};
      data[$(elem).attr('name')] = [];
      if (_this.Mobile) {
        $(elem).find('.tbody .tr').each(function (i, el) {
          ...
          $(el).find('.phone-form-group').each(function (j, child) {
            var typeHtml = $(child).attr('data-xhtml') == 'text' ? 'input' : $(child).attr('data-xhtml') == 'datetimepicker' ? 'input' : 'select';
    ...
    getdatachild: function getdatachild(elem) {
      var _this = this;
      var data = {};
      $(elem).find('.group').each(function (i, el) {
        var obj = _this.getdata[$(el).attr('data-xhtml')](el);
        Object.assign(data, obj);
      });
      return data;
    },
    getTime: function getTime($tdHtml, option) {
      if (option.lang = 'ch') {
        option.lang = 'zh';
      }
    ...
    regFn: function regFn(id) {
      $(id).validator({
        onValid: function onValid(validity) {
          $(validity.field).closest('.am-form-group').find('.am-alert').hide();
        },

        onInValid: function onInValid(validity) {
  ...
  authorize.phoneTable = {
    ...
    loadTable: function loadTable(page, eldata, elattribute) {
      var _this5 = this;

      var $html = $(this.phoneTable.tableHtml);
      var _this = this;
      ...
      var collect = [];
      page.content.forEach(function (el) {
        var tdData = '';
        ...
        if (elattribute[el.name].ifCollect) {
          ...
          var $divTd = _this5.phoneTable.addChild(_this, el, '', elattribute[el.name]);
        ...
        if (eldata[page.name] && eldata[page.name].length) {
          eldata[page.name].forEach(function (v, i) {
            tdData = v[el.name];
            var $divTd = _this5.phoneTable.addChild(_this, el, tdData, elattribute[el.name]);
          ...
          var _$divTd = _this5.phoneTable.addChild(_this, el, '', elattribute[el.name]);
          if ($html.find('.tbody .tr')[0]) {
            $html.find('.tbody .tr').eq(0).append(_$divTd);
          } else {
            $html.find('.tbody').append('<div class="tr content am-g"></div>');
            $html.find('.tbody .tr').eq(0).append(_$divTd);
          }
      ...
      collect.forEach(function (v) {
        var sum = 0;
        $html.find('.tbody .tr [name = ' + v + ']').each(function (i, e) {
    ...
    addtr: function addtr(_this, page, elattribute) {
      $('.tbody .tr').removeClass('active');
      // $('.iconDelete').hide()
      var $divTr = $('<div class="tr content am-g active"><div class="iconDelete"><i class="am-icon-close"></i></div></div>');
      page.content.forEach(function (el) {
        $divTr.append(_this.phoneTable.addChild(_this, el, '', elattribute[el.name]));
      });
      $(this).parent().find('.tbody').append($divTr);
    },
    addChild: function addChild(_this, el, tDdata, tDattribute) {
      if (el.type == 'text') {
        var $tdHtml = $('<div class="am-form-group  phone-form-group am-u-sm-12 " data-xhtml="text">\n        <div class="phone-group">\n          <div class ="phone-label">\n            <label for="" class="title "><span></span>:</label>\n          </div>\n          <input type="text" id="" class="phone-item" minlength="" placeholder="" value="" data-validation-message="" pattern=""/>\n        </div>\n      </div>');
        ...
        var _$tdHtml = $('<div class="am-form-group phone-form-group am-u-sm-12" data-xhtml="select">\n        <div class="phone-group">\n          <div class="phone-label">\n              <label for="" class="title"><span>\u4EA4\u901A\u5DE5\u5177</span>:</label>\n            </div>\n            <div style="background:#fff;width:70%">\n              <select data-am-selected="{btnWidth: \'100%\'}">\n              </select>\n            </div>\n        </div>\n      </div>');
        _$tdHtml.find('.title span').text(el.titleTh);
        _$tdHtml.find('select').attr({ 'name': el.name });
        el.data.value.forEach(function (v) {
          var option = '<option value="' + v.value + '">' + v.value + '</option>';
          var $option = $(option);
          if (tDdata) {
            if (v.value == tDdata) {
          ...
          _$tdHtml.find('select').append($option);
        });
        if (tDattribute.ifWrite) {
          _$tdHtml.find('.title').append('<sup class="am-text-danger">*</sup>');
          _$tdHtml.find('select').attr('required', true);
        }
        if (!tDattribute.ifShow) {
          _$tdHtml.find('select').replaceWith('<div class="phone-item" style="text-align: center;width:100%"> —— </div>');
        }
        if (!tDattribute.ifEditor) {
          _$tdHtml.find('select').attr('disabled', true);
        }
        return _$tdHtml;
      } else {
        var _$tdHtml2 = $('<div class="am-form-group phone-form-group am-u-sm-12 " data-xhtml="datetimepicker">\n        <div class="phone-group">\n          <div class ="phone-label">\n            <label for="" class="title "><span></span>:</label>\n          </div>\n          <input type="text"class="phone-item"  readonly="readonly"/>\n        </div>\n        \n      </div>');
        _$tdHtml2.find('.title span').text(el.titleTh);
        _$tdHtml2.find('input').attr('name', el.name);
        el.data.value = el.data.value.replace(/\#\{getToday\(\)\}/g, authorize.Format(new Date(), /^Y+.m+.d+/i.exec(el.data.option.format)[0]));
        if (tDdata) {
          _$tdHtml2.find('input').attr({ 'value': tDdata });
        } else {
          _$tdHtml2.find('input').attr({ 'value': el.data.value });
        }
        if (tDattribute.ifWrite) {
          _$tdHtml2.find('.title').append('<sup class="am-text-danger">*</sup>');
          _$tdHtml2.find('input').attr('required', true);
        }

        if (!tDattribute.ifShow) {
          _$tdHtml2.find('input').replaceWith('<div class="phone-item" style="text-align: center;"> —— </div>');
        }
        if (!tDattribute.ifEditor) {
          _$tdHtml2.find('input').attr('disabled', true);
        } else {
          // $.datetimepicker.setLocale(el.data.option.lang);
          // $tdHtml.find('input').datetimepicker(el.data.option)
          _this.getTime(_$tdHtml2, el.data.option);
        }
        return _$tdHtml2;
      }
    }
  ...
  authorize.table = {
    ...
    loadTable: function loadTable(page, eldata, elattribute) {
      ...
      page.content.forEach(function (el) {
        ...
        if (eldata[page.name] && eldata[page.name].length) {
          eldata[page.name].forEach(function (v, i) {
            tdData = v[el.name];
            var $tD = _this.table.ChildHtml(_this, el, tdData, elattribute[el.name]);
            var $tDHtml = $('<td></td>');
            $tDHtml.append($tD);
            if ($html.find('tbody tr')[i]) {
    ...
    addTH: function addTH(_this, page, elattribute) {
      $('tbody tr').removeClass('active');
    ...
    ChildHtml: function ChildHtml(_this, el, tDdata, tDattribute) {
      if (el.type == 'text') {
        var $text = $('<input type="text" id="" placeholder="" value="" data-validation-message="" pattern=""/>');
        el.data.value = el.data.value.replace(/^\#{user.name}$/g, _this.formData.cache.user.name || el.data.value);
        el.data.value = el.data.value.replace(/^\#{org.orgName}$/g, _this.formData.cache.org.orgName || el.data.value);
        $text.attr({
          'name': el.name,
        ...
        var $form = $('<div class="am-form-group">\n                </div>');
        $form.append($text);
        return $form;
      } else if (el.type == 'select') {
        var $select = $('<div class="am-form-group" style="background:#fff"><select data-am-selected="{btnWidth: \'100%\'}">\n      </select>\n      </div');
        ...
        var $datetimepicker = $('<div class="am-form-group"><input  type="text" id=""  placeholder="" class="am-form-field nameValue input datetimepicker readonly"/></div>');
  ...
  authorize.text = {
    ...
    loadText: function loadText(page, eldata, elattribute) {
      var $html = '';
      ...
      if ((typeof eldata === 'undefined' ? 'undefined' : _typeof(eldata)) === 'object' && eldata[page.name]) {
        $html.find('input').attr('value', eldata[page.name]);
      }
      if ((typeof elattribute === 'undefined' ? 'undefined' : _typeof(elattribute)) === 'object' && elattribute[page.name]) {
  ...
  authorize.textarea = {
    ...
    textareaHtml4: '\n  <div class="am-form-group group am-u-sm-12" data-xhtml="textarea">\n    <div  class="authorize-form-row2">\n      <label for="" class="title am-u-sm-3"><span></span>:</label>\n      <textarea id="" class="gridContent"></textarea>\n      <div class="subhead am-u-sm-4"></div>\n    </div>\n  </div>',
    textareaHtml5: '\n    <div class="am-form-group group am-u-sm-12" data-xhtml="textarea">\n      <label for="" class="title"><span></span>:</label>\n      <textarea id=""></textarea>\n    </div>',
    loadTextarea: function loadTextarea(page, eldata, elattribute) {
      var $html = '';
      if (page.ComponentType == 'OneRowAndTwoColumns') {
  ...
  authorize.radio = {
    ...
    radioHtml4: '\n  <div class="am-form-group group am-u-sm-12" data-xhtml="radio">\n    <div class="authorize-form-row3">\n      <h3 class="title "><span></span>:</h3>\n      <div class="label gridContent"></div>\n      <div class="subhead"></div>\n    </div>\n  <div>',
    radioHtml5: '\n  <div class="am-form-group group am-u-sm-12" data-xhtml="radio">\n    <h3 class="title"><span></span>:</h3>\n    <div class="label"></div>\n  <div>',
    loadRadio: function loadRadio(page, eldata, elattribute) {
      var $html = '';
      if (page.ComponentType == 'OneRowAndTwoColumns') {
      ...
      $html.find('.subhead').text(page.subhead);
      page.data.value.forEach(function (v) {
  ...
  authorize.checkbox = {
    ...
    checkboxHtml4: '\n  <div class="am-form-group group am-u-sm-12" data-xhtml="checkbox">\n    <div class="authorize-form-row3">\n      <h3 class="title"><span></span></h3>\n      <div class="label gridContent"></div>\n      <div class="subhead"></div>\n    </div>\n  </div>',
    checkboxHtml5: '\n  <div class="am-form-group group am-u-sm-12" data-xhtml="checkbox">\n    <h3 class="title"><span></span></h3>\n    <div class="label"></div>\n  </div>',
    loadCheckbox: function loadCheckbox(page, eldata, elattribute) {
      var $html = '';
      if (page.ComponentType == 'OneRowAndTwoColumns') {
      ...
      page.data.value.forEach(function (v) {
        ...
        var $label = $(label);
        if (eldata[page.name]) {
          eldata[page.name].forEach(function (e) {
            if (e === v.value) {
              $label.find('input').attr('checked', true);
            }
          });
  ...
  authorize.select = {
    ...
    loadSelect: function loadSelect(page, eldata, elattribute) {
      var $html = '';
      if (page.ComponentType == 'OneRowAndTwoColumns') {
  ...
  authorize.datetimepicker = {
    ...
    loadDatetimepicker: function loadDatetimepicker(page, eldata, elattribute) {
      var $html = '';
      if (page.ComponentType == 'OneRowAndTwoColumns') {
  ...
  authorize.file = {
    ...
    loadFile: function loadFile(page, eldata, elattribute) {
      var $html = '';
      if (page.ComponentType == 'OneRowAndTwoColumns') {
      ...
      if (eldata[page.name]) {
        eldata[page.name].forEach(function (v) {
          var $fileItem = $('<div class="file-item" data-dataurl="">\n              <div class="file-left">\n                <div class="file-title"><a download="" target="_blank" href="">\u5929\u5929.text</a></div>\n                <div class="file-size">256kb</div>\n              </div>\n              <div class="file-right">\n                <i class="am-icon-trash"></i>\n              </div>\n            </div>');
          var size = v.size / 1024 > 1024 ? (v.size / 1024 / 1024).toFixed(2) + 'M' : (v.size / 1024).toFixed(2) + 'KB';
    ...
    getFile: function getFile(page, _this) {
      var filethis = this;
      var data = new FormData();
      var url = coos.getRootPath() + 'fileUpload/upload.do';
      ...
      $.ajax({
        ...
        success: function success(res) {
          ...
          for (var j = 0; j < res.result.length; j++) {
            ...
            var file = res.result[j];
            var size = file.size / 1024 > 1024 ? (file.size / 1024 / 1024).toFixed(2) + 'M' : (file.size / 1024).toFixed(2) + 'KB';
    ...
    preview: function preview(page, _this) {
      var _this6 = this;
  ...
  authorize.image = {
    ...
    loadImage: function loadImage(page, eldata, elattribute) {
      var _this7 = this;
      ...
      if (eldata[page.name]) {
        eldata[page.name].forEach(function (v) {
          ...
          _this7.image.btnImg($imgItem, v.dataUrl);
    ...
    btnImg: function btnImg($imgItem, url) {
    ...
    getImage: function getImage(page, _this) {
      var thisImage = this;
      var data = new FormData();
      var url = coos.getRootPath() + 'fileUpload/upload.do';
      if (!this.files.length) {
      ...
      $.ajax({
        ...
        success: function success(res) {
          ...
          for (var j = 0; j < res.result.length; j++) {
            var _$fileItem = $('<div class="file-item" data-dataurl="">\n            <img src=\'\' class="img-left"/>\n            <div class="file-content">\n              <div class="file-title">ttkkk.png</div>\n              <div class="file-size">785KB</div>\n            </div>\n            <div class="file-right">\n              <i class="am-icon-trash "></i>\n            </div>\n          </div>');
            var _file = res.result[j];
            var size = _file.size / 1024 > 1024 ? (_file.size / 1024 / 1024).toFixed(2) + 'M' : (_file.size / 1024).toFixed(2) + 'KB';
            _$fileItem.find('.img-left').attr('src', _file.dataUrl);
            _$fileItem.find('.file-title').text(_file.name);
            _$fileItem.find('.file-size').text(size);
            _$fileItem.attr('data-dataurl', JSON.stringify(_file));
            _this.image.btnImg(_$fileItem, reader.result);
            _$fileItem.find('i').click(function () {
              $(this).parent().parent().remove();
              if (!$('#' + page.id).find('.file-item').length) {
                $('#' + page.id).find('input').val('');
                if (!page.data.ifWrite) {
            ...
            $('#' + page.id).find('.file-list').append(_$fileItem);
          }
        }
      });
    }
  ...
  authorize.user = {
    ...
    userLoad: function userLoad(page, eldata, elattribute) {
      var _this9 = this;

      var _this = this;
      var $html = $(this.user.userHtml);
      $html.addClass('am-u-md-' + page.grid);
      page.id += 'user';
      if (elattribute[page.name]) {
        page.data.ifWrite = elattribute[page.name].ifWrite;
      ...
      page.data.value.forEach(function (v) {
        if (v.userID == '#{user.name}') {
          v.name = _this9.formData.cache.user.name;
          v.userID = _this9.formData.cache.user.userID;
        }
    ...
    addUser: function addUser(userData, condition) {
      ...
      $('#' + userData.id).find('.user-item').each(function (i, v) {
  ...
  authorize.organize = {
    ...
    organizeLoad: function organizeLoad(page, eldata, elattribute) {
      var _this10 = this;

      var _this = this;
      var $html = $(this.organize.organizeHtml);
      page.id += 'organize';
      $html.addClass('am-u-md-' + page.grid);
      if (elattribute[page.name]) {
        page.data.ifWrite = elattribute[page.name].ifWrite;
    ...
    addOrganize: function addOrganize(organizeData, condition) {
      ...
      var $defaultHtml = $(defaultHtml);
      var datali = '';
      $('#' + organizeData.id).find('.organize-item').each(function (i, v) {
  ...
  //   }
  // })
  authorize.userChoice = function (_ref) {
    var ifChoice = _ref.ifChoice,
        userArr = _ref.userArr,
        sureFn = _ref.sureFn;

    var userChoiceName = 'userChoiceName' + authorize.app.getNumber();
    var defaultHtml = '<div id="user-box">\n          <div class="user-model">\n            <div class="user-title"><span>\u7528\u6237\u5217\u8868</span> <i class="am-icon-close back"></i></div>\n            <div class="ul-box">\n              <ul class="list-selecred">\n              </ul>\n            </div>\n            <div class="user-list">\n              <div class="list-search">\n                <input class="input-search">\n                <i class="am-icon-search search"></i>\n                <div class="searchListBox" style="display: none">\n                  <ul class="searchListUl">\n                    <li class="searchItem">linlinlin</li>\n                    <li class="searchItem">linlinlin</li>\n                  </ul>\n                </div>\n              </div>\n              <div class="list-name">\n                <ul class="list-ul">\n                </ul>\n              </div>\n            </div>\n            <div class="btn-box">\n              <div>\n                <button type="button" class="am-btn am-btn-default am-radius previous">\u4E0A\u4E00\u9875</button>\n                <button type="button" class="am-btn am-btn-default am-radius next">\u4E0B\u4E00\u9875</button>\n              </div>\n              <div>\n                <button type="button" class="am-btn am-btn-default am-radius back" >\u53D6\u6D88</button>\n                <button type="button" class="am-btn am-btn-success am-radius sure">\u786E\u5B9A</button>\n              </div>\n            </div>\n          </div>\n        </div>';
    var $defaultHtml = $(defaultHtml);
    var resData = [{ name: "loo1", userID: "21256832676" }, { name: "loo2", userID: "21256832675" }, { name: "loo4", userID: "21256832677" }, { name: "loo3", userID: "21256832673" }, { name: "loo5", userID: "21256832674" }];
    function getuser(page) {
    ...
    userArr.forEach(function (v) {
      var li = '<li><span class="userName" data-value=' + JSON.stringify(v) + ' value="' + v.name + '">' + v.name + '</span> <span class="del-user">x</span></li>';
  ...
  //   ifChoice: 'radio'
  // })
  authorize.organizeChoice = function (_ref2) {
    var ifChoice = _ref2.ifChoice,
        orgData = _ref2.orgData,
        sureFn = _ref2.sureFn;
    ...
    $defaultHtml.find('.sure').click(function () {
      var ulLiData = [];
      $('#organize-box .userName').each(function (i, v) {
        ulLiData.push(JSON.parse($(v).attr("data-value")));
      });
      sureFn(ulLiData);
      $('#organize-box').remove();
  ...
  authorize.Format = function (detaTime, fmt) {
    ...
    if (/(y+)/i.test(fmt)) fmt = fmt.replace(RegExp.$1, detaTime.getFullYear() + "");
    for (var k in o) {
      if (new RegExp("(" + k + ")", 'i').test(fmt)) fmt = fmt.replace(RegExp.$1, ("00" + o[k]).substr(("" + o[k]).length));
    }return fmt;
  };
