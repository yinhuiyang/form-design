			...
			if (condition.minLangth > parseInt($(this).val())) {
				if (!$(this).parent().parent().find('.am-alert')[0]) {
					$(this).parent().parent().append(
						'<div class=\'am-alert am-alert-danger\' style=\' display: block;\'>最小长度不能大于最大长度</div>')
				}
			} else {
				condition.maxLangth = parseInt($(this).val())
				$(`#${id}`).attr('data-xhtml') == 'table' ? $('#' + id).find('th.active').attr('data-text', JSON.stringify(
					condition)) : $(`#${id}`).attr('data-xdata', JSON.stringify(condition))
	...
	textInput(id, type) {
      ...
      <div>
		...
		if ($(`#${id}`).attr('data-xhtml') == 'textarea') {
			let condition = JSON.parse($(`#${id}`).attr('data-xdata'))
			inputHtml +=
				`<textarea  id="default" rows="2" maxlength="${condition.maxLangth}" onchange="${iptvalue};iptvalue.call(this, '${id}')" oninput="${iptvalue};iptvalue.call(this, '${id}')"  placeholder="" class="am-form-field">${$('#'+id).find('.input').val()}</textarea>`
		} else if ($(`#${id}`).attr('data-xhtml') == 'datetimepicker') {
			inputHtml +=
				`<input type="text" id="default" class="am-form-field custom datetimepicker" onchange="${iptvalue};iptvalue.call(this, '${id}')" value="${$('#'+id).find('.input').val()}"/>` +
				`<i class="am-icon-question icon-bz" onclick='${defaultFn};defaultFn("${id}", "${type}")'></i>`
		} else if ($(`#${id}`).attr('data-xhtml') == 'table') {
			if (type == 'data-text') {
				condition = JSON.parse($('#' + id).find('th.active').attr('data-text'))
				inputHtml +=
					`<input type="text" id="default" maxlength="${condition.maxLangth}" class="am-form-field custom" onchange="${iptvalue};iptvalue.call(this, '${id}')" oninput="${iptvalue};iptvalue.call(this, '${id}')" value="${(JSON.parse($('#'+id).find('th.active').attr(type))).value}"/>` +
					`<i class="am-icon-question icon-bz" onclick='${defaultFn};defaultFn("${id}")'></i>`
			} else {
				inputHtml +=
					`<input type="text" id="default" onchange="${iptvalue};iptvalue.call(this, '${id}')" class="am-form-field custom" oninput="${iptvalue};iptvalue.call(this, '${id}')" value="${(JSON.parse($('#'+id).find('th.active').attr(type))).value}"/>` +
					`<i class="am-icon-question icon-bz" onclick='${defaultFn};defaultFn("${id}", "${type}")'></i>`
			}
		} else {
			let condition = JSON.parse($(`#${id}`).attr('data-xdata'))
			inputHtml +=
				`<input type="text" id="default" maxlength="${condition.maxLangth|| ''}" class="am-form-field custom" onchange="${iptvalue};iptvalue.call(this, '${id}')"  oninput="${iptvalue};iptvalue.call(this, '${id}')" value="${$('#'+id).find('.input').val()}"/>` +
				`<i class="am-icon-question icon-bz" onclick='${defaultFn};defaultFn("${id}")'></i>`
		}
		inputHtml += '</div>'
		inputHtml +=
      ...
      <div class="am-form-group">`
		if ($(`#${id}`).attr('data-xhtml') == 'table') {
			inputHtml +=
				`<input type="text" id="prompt" oninput="${iptPlaceholder};iptPlaceholder.call(this, '${id}')" value="${(JSON.parse($('#'+id).find('th.active').attr(type))).placeholder}" class="am-form-field custom"/>
      </div>`
		} else {
			inputHtml +=
			...
			if ($(`#${id}`).attr('data-xhtml') == 'table') {
				if ($('#' + id).find('th.active').attr('data-type') == 'text') {
					...
					if ($(this).val().length < data.minLangth && $(this).val().length != 0) {
						$(this).parent().parent().find('.am-alert')[0] ? $(this).parent().parent().find('.am-alert').remove() : $(this).parent()
							.parent().append('<div class=\'am-alert am-alert-danger\' style=\' display: block;\'>最少输入' + data.minLangth +
								'个字符</div>')
						$(this).parent().parent().find('.am-alert')[0] ? '' : $(this).parent().parent().append(
							'<div class=\'am-alert am-alert-danger\' style=\' display: block;\'>最少输入' + data.minLangth + '个字符</div>')
					} else {
						$(this).parent().parent().find('.am-alert').remove()
						data.value = $(this).val()
				...
				if ($(`#${id}`).attr('data-xhtml') == 'text' || $(`#${id}`).attr('data-xhtml') == 'textarea') {
					if ($(this).val().length < condition.minLangth && $(this).val().length != 0) {
						$(this).parent().parent().find('.am-alert')[0] ? $(this).parent().parent().find('.am-alert').remove() : $(this).parent()
							.parent().append('<div class=\'am-alert am-alert-danger\' style=\' display: block;\'>最少输入' + condition.minLangth +
								'个字符</div>')
						$(this).parent().parent().find('.am-alert')[0] ? '' : $(this).parent().parent().append(
							'<div class=\'am-alert am-alert-danger\' style=\' display: block;\'>最少输入' + condition.minLangth + '个字符</div>')
					} else {
						$(this).parent().parent().find('.am-alert').remove()
		...
		function defaultFn(id, type) {
			let html =
				`<div class="am-modal am-modal-no-btn" tabindex="-1" id="">
        <div class="am-modal-dialog">
          ...
          <ul class="am-list am-list-static">`
			if ($(`#${id}`).attr("data-xhtml") === "text" || $("#" + id).find("th.active").attr("data-type") == "text") {
				html += `<li><span class="default-value">` + "#{user.name}" +
					`</span><span class="am-badge am-badge-success">输入</span></li></li>
          <li><span class="default-value">` +
					"#{org.orgName}" + `</span><span class="am-badge am-badge-success">输入</span></li></li>`
			}
			if ($(`#${id}`).attr("data-xhtml") === "datetimepicker" || $("#" + id).find("th.active").attr("data-type") ==
				"datetimepicker") {
				html += `<li><span class="default-value">` + "#{getToday()} </span><input type=" + "text" + " class=" +
					"datetimepicker-time" + " " + "/>" +
			...
			if ($(`#${id}`).attr("data-xhtml") === "datetimepicker" || $("#" + id).find("th.active").attr("data-type") ==
				...
				$html.find(".datetimepicker-time").datetimepicker({
					lang: "ch",
					step: step,
					datepicker: false,
					timepicker: true,
